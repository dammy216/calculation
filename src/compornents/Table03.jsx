...
const Table03 = ({subject, unit, setValue03}) => {
  
  const [count03,setCount03] = useLocalStorage("count03", 0);
  ...
  const handleClick = (action) => {
    ...
    switch (action.type) {
      case "plus2":
          setCount03(count03+2);
        break;
      case "plus1":
          setCount03(count03+1);
        break;
        case "minus2":
          setCount03(Math.max(count03-2,0))
        break;
        case "minus1":
          setCount03(Math.max(count03-1,0))
          break;
      default:
        break;
      }
  ...
  return (
    <div>
      <table border={1}>
        ...
        <td rowSpan={2}>{count03}</td>
        <td rowSpan={2}>{Math.max(unit-count03,0)}</td>
        </tr>
        <tr>
