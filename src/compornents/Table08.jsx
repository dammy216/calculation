...
const Table08 = ({subject, unit, setValue08}) => {

  const [count08,setCount08] = useLocalStorage("count08", 0);
  ...
  const handleClick = (action) => {
    ...
    switch (action.type) {
      case "plus2":
          setCount08(count08+2);
        break;
      case "plus1":
          setCount08(count08+1);
        break;
        case "minus2":
          setCount08(Math.max(count08-2,0))
        break;
        case "minus1":
          setCount08(Math.max(count08-1,0))
          break;
      default:
        break;
      }
  ...
  return (
    <div>
      <table border={1}>
        ...
        <td rowSpan={2}>{count08}</td>
        <td rowSpan={2}>{Math.max(unit-count08,0)}</td>
        </tr>
        <tr>
