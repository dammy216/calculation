...
const Table06 = ({subject, unit, setValue06}) => {

  const [count06,setCount06] = useLocalStorage("count06", 0);
  ...
  const handleClick = (action) => {
    ...
    switch (action.type) {
      case "plus2":
          setCount06(count06+2);
        break;
      case "plus1":
          setCount06(count06+1);
        break;
        case "minus2":
          setCount06(Math.max(count06-2,0))
        break;
        case "minus1":
          setCount06(Math.max(count06-1,0))
          break;
      default:
        break;
      }
  ...
  return (
    <div>
      <table border={1}>
        ...
        <td rowSpan={2}>{count06}</td>
        <td rowSpan={2}>{Math.max(unit-count06,0)}</td>
        </tr>
        <tr>
